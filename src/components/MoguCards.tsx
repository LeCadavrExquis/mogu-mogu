...
const GottaChewCard: FC = () => {
    ...
    ><MoguCard
        ...
        sx={{width: screen?.width! > 800 ? 600 : 360}}
    >
...
const NataDeCocoCard: FC = () => {
    ...
    return <MoguCard
        ...
        className={"nata"}
        sx={{width: screen?.width! > 1000 ? 1000 : 360}}
    >
        <Stack direction={"column"} justifyItems={"end"}>
            ...
            </Typography>
            {
                isCardExpanded && <>
                    ...
                    <Typography paragraph={true}>
                        Najbardziej tajemniczy i zaskakujący składnik Mogu Mogu, w dużej mierze decydujący o jego oryginalności, to nata de coco.
                        Nazwa ta, w dosłownym tłumaczeniu z języka hiszpańskiego oznacza „śmietankę kokosową”, jednak częściej używa się określenia
                        „galaretka kokosowa” (ang. coconut jelly), które dość dobrze opisuje konsystencję tego produktu.
                        Nata de coco wywodzi się z Filipin, zyskała jednak dużą popularność także w Azji południowo-wschodniej i innych egzotycznych
                        zakątkach świata. Galaretka powstaje w procesie fermentacji wody kokosowej, odbywającej się przy udziale dobrych kultur
                        bakterii Acetobacter xylinum. Woda kokosowa, nazywana także sokiem z kokosa, to płyn znajdujący się we wnętrzu młodych, jeszcze zielonych orzechów kokosowych.
                        Jest produktem całkowicie naturalnym i pozbawionym jakichkolwiek zanieczyszczeń, gdyż w drodze od korzeni do owoców woda poddawana jest wielokrotnej filtracji wewnątrz drzewa.
                    </Typography>
                    <Typography paragraph={true}>
                        Poza walorami zdrowotnymi, Nata de Coco, dzięki swej niezwykłej, żelowej konsystencji stała się składnikiem popularnych w tropikach deserów,
                        a połączona z sokiem owocowym w napoju Mogu Mogu daje niezwykłe, nieporównywalne z niczym innym doznania smakowe.
                        Ponadto, ze względu na skład zbliżony do składu ludzkiego osocza, jest napojem znakomicie przyswajalnym przez nasz organizm.
                        Również z tego powodu, w czasie II wojny światowej była ona podawana w kroplówkach, jako substytut transfuzji krwi, r
                        annym podczas walk w rejonie Pacyfiku. Wśród licznych, niezbędnych człowiekowi mikroelementów, które zawiera wystarczy wymienić chociażby:
                        potas, fosfor, magnez i wapń, żelazo oraz kwas foliowy, a także witaminę C, czy witaminy z grupy B. Jest produktem niskokalorycznym,
                        bezpiecznym dla alergików, zawiera dobry cholesterol i opóźniające procesy starzenia antyoksydanty (w tym kinetynę).
                        Wspomaga układ odpornościowy. Organizacja Narodów Zjednoczonych do Spraw Wyżywienia i Rolnictwa (FAO) określiła wodę kokosową jako bogaty w elektrolity, naturalny napój izotoniczny.
                        Te liczne zalety powodują, że woda kokosowa stała się popularnym napojem orzeźwiającym w krajach tropikalnych, gdzie podawana jest często wprost z nacinanych maczetą kokosów,
                        a także została doceniona przez znane z troski o zdrowe celebrytki takie jak Madonna czy Demi Moore. Nata de Coco, jako produkt wytwarzany z wody kokosowej w procesie naturalnej fermentacji, zachowuje jej właściwości, a ponadto, dzięki dodatkowej zawartości błonnika przyspiesza przemianę materii, a także, w najlepszy z możliwych sposób, oczyszcza jelita, dzięki czemu pozwala uniknąć wielu chorób.
                    </Typography>
                </>
            }
            <Stack>
...
const FindUs: FC = () => {
    ...
    const ytOpts: YouTubeProps['opts'] = {
        height: screen?.width! > 640 ? '360' : '180',
        width: screen?.width! > 640 ? '640' : '320',
        playerVars: {
    ...
    return <MoguCard
        className={"findUs"}
        title={"ZNAJDŹ NAS!"}
        sx={{width: screen?.width! > 1200 ? 1200 : 360}}
    >
        <Stack direction={screen?.width! > 1000 ? "row" : "column"} justifyItems={"end"}>
            <YouTube
                videoId={"FcJzUyFmJPM"}
                opts={ytOpts}
            />
            <Box sx={{paddingLeft: 4, paddingTop: 4}}>
                <Stack direction={"column"}>
                    <Typography paragraph={true}>
                        Napój znajdziesz w sieciach sklepów Kuchnie Świata, Auchan, Aldi. Dostępy jest również na stacjach paliw Shell, warszawskich hurtowniach Makro i Kamix
